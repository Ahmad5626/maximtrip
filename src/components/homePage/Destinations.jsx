      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="hidden md:block">
            <button className="flex items-center space-x-2 text-blue-500 hover:text-blue-600 font-medium transition-colors group">
             <Link to="/alldestination" className="flex items-center space-x-2">
        ...
        <div className="relative">
          <Swiper
            ...
            loop={true}
            className="destinationsData-swiper"
          >
            {destinationsData.filter((item) => item.status === "Active")?.map((blog) => (
              <SwiperSlide key={blog._id}>
                <Link to={`/destination/${blog.slug}`}>
